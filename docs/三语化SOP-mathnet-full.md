...
小批通过后去掉 `--limit` 与 `--strict`。`--help` 中的默认值是每批 25 题、4 个并发；按第 8 节
的质量与耗时实测，全量也推荐每批 25 题、6～8 个并发。下面取并发 8：
...
推荐配置是批 25、并发 6～8，预计约 37～49 小时。A/B 实测中，批 100 会让长解法退化成套话，
批 25 则通过全部质量门禁：单响应的输出预算达到几万 token 时，模型更容易用套话压缩长解法，
所以这里优先保证译文完整性，不用更大的批量换取吞吐。批 100 是耗时实测点，批 200 只是外推；
两者都不作为推荐值。
