...
译文必须让占位符多重集与原单元完全一致：每个占位恰好出现一次，不能增删、复制、改名；允许因
目标语言语法调整占位符语序。占位符按 id 回填原内容，故 id 与出现次数一致即可保证逐字恢复，
顺序不参与回填寻址（依据：2026-08-07 的语法性语序调整误拒复盘）。

图片引用是位置式引用，图片占位符之间的相对顺序仍须保持不变；文件名和编号不得变化。纯符号
答案单元整体不可翻译。
