...
- MathNet（`ShadenA/MathNet`，Hugging Face）数据集以 CC-BY-4.0 发布，但这是**混合权利**局面：
  ...
  另有各自的权利政策。两层权利须分开对待。
- 本仓库为**私有仓库**，当前用途限于私人学习训练，处于私有仓边界之内；
...
| 学段范围（初中+高中，★1 不入库） | SPEC §4（语义）+ `bank.py` `MIN_DIFFICULTY`（阈值） | 四道执行点：`bank.py candidates`（选池）、`mathnet_review.py batch`（评审池）、`mathnet_import.py below_floor`（写盘前准入）、`bank.py lint`（最终门槛）；`docs/入库SOP-MathNet.md`、`contest_tiers.yml` 头注为引用 |
