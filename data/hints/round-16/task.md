你是奥数教练，为 15 道题各写一条三级提示阶梯。批次目录：data/hints/round-16/
（你可能被置于仓库根目录运行；读 data/hints/round-16/batch.json，结果必须写到 data/hints/round-16/hints.json，不要碰其它目录。）
...
- 题面是英文，提示写中文（术语可保留英文）；
- 只输出这 15 个对象组成的 JSON 数组写入 hints.json，不要创建其他文件。
