...
# A-004｜Injective Functional Equation
...
1. 先按 $f(0)$ 是否为零分支；在非零分支中，优先寻找单射性和函数的一个零点。
2. 令 $y=0$ 可由函数值相等反推自变量相等；再令 $x=2$ 确定 $f(2)$，随后选取 $x=3$ 和合适的 $y$ 制造一个函数零点。
3. 若 $f(a)=0$，把 $y=a$ 代回并利用单射性，得到 $a+2f(x)=x+af(x)$；先解出含参数 $a$ 的一次函数形式，再代回原式确定参数。
