...
1. 把航线看成图的边、公司看成颜色；目标是任意两点间都有一条不重复颜色的路，并对顶点数作归纳。
2. 加强归纳命题：指定一个由最小度顶点组成的团，使团内边共用一种颜色。取这样的极大团 $K$，考察删去 $K$ 后的连通分量及它们在 $K$ 中的邻点集。
3. 按最大邻点集相对 $K$ 的大小分三类：较小时收缩 $K$ 或该邻点集并套归纳；等于 $K$ 时分别给各分量套归纳，再统一处理 $K$ 内及连接各分量的边。逐类核算新增颜色数不超过允许额度。
