...
def _has_prose_outside_target_language(
    ...
    中文沿用已验证的目标语言口径；英文恢复 ``und``、拉丁变音符、片段词与
    具体语言检测，但调用点必须先证明小节逐字相同。数学、图片、纯符号与生成器
    占位仍由 ``_has_translatable_prose`` 先行排除。
    """
    if not _has_translatable_prose(section):
        return False

    return _target_language_mismatch(
...
def _language_findings(
    ...
    """检查改写后中文覆盖与所有译文乱码，不重复逐字相同 finding。"""
    source_sections = _sections(source)
    ...
    for index, section in enumerate(translated_sections):
        ...
        if matching_source and not _has_translatable_prose(source_section):
            continue
        if not _has_translatable_prose(section):
            continue
